...
public class Queue{
	...
	void SeizeFirstQueueObject(int amountOfTimeToSeize)
	{
		...
		//Jennifer test
		if(firstQueueObject.GetGroupSize() <= 1) {
	     // when there is only onw peron, the sigle record get, then need to be added
		firstQueueObject.GetWaitingTime();
		}else {
			//when there are multi people in a group for exmple 3 person, the 3 times record be added.
			int people = firstQueueObject.GetGroupSize();
			double waitingtimeofpeople = firstQueueObject.GetWaitingTime();
			// thw result must be added
		}
